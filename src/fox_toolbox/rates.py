...
class MidCurve(Swaption):
	def __init__(self, correl, meanR, expiry, vol, start_date, pmnt_dates, dcfs, libor_tenor, **kwargs):
		super().__init__(expiry, vol, start_date, pmnt_dates, dcfs, libor_tenor)
		self._correl = float(correl)
		self._meanR = float(meanR)
